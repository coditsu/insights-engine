...
RSpec.describe InsightsEngine::Harvesters::GitInspector::Harvester do
  ...
  describe '#process' do
    ...
    it { expect { JSON.parse(output[:stdout]) }.not_to raise_error }
    it { expect(JSON.parse(output[:stdout]).keys.count).to eq(1) }
    it { expect(JSON.parse(output[:stdout])).to have_key('gitinspector') }
