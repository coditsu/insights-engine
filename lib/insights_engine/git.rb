...
module InsightsEngine
  ...
  class Git
    class << self
      # Returns a shortlog describing all the commiters for project that is under build_path
      ...
      # Returns an information about last commiter that changed/created this file
      # We use it to blame users for file related errors, assuming that the fault goes always
      # to a person that changed the file last and didn't correct or created a particular error
      # @param build_path [String] path of a current repository build
      ...
      def shell(build_path, command, arguments)
        ...
        raise Errors::FailedGitCommand, "#{command_with_options}: #{result[:stderr]}"
      end
    end
  end
end
